...
class GoodRepository extends BaseRepository
{
     ...
     * @return array
     */
    public function getGoodsByRelatedTypeAndOs($user, int $os, string $appName, int $platform, int $relatedType)
    {
        // 半月会员与周会员AB测试
//        $isHalfMonthTest = false;
//        if ($user instanceof User) {
//            $isHalfMonthTest = rep()->userAb->getQuery()->where('user_id', $user->id)
//                ->where('type', UserAb::TYPE_HALF_MONTH_TEST_B)->count();
//        }
//        $exceptCardLevel = $isHalfMonthTest ? Card::LEVEL_WEEK : Card::LEVEL_HALF_MONTH;
//        $exceptCardLevel = Card::LEVEL_WEEK;
//        $exceptCardIds   = rep()->card->getQuery()->where('level', $exceptCardLevel)
//            ->pluck('id')->toArray();
        $exceptCardIds = [];
        if ($user instanceof User && $user->getRawOriginal('gender') == User::GENDER_WOMEN) {
            $exceptCardLevel = Card::LEVEL_HALF_MONTH;
            $exceptCardIds   = rep()->card->getQuery()->where('level', $exceptCardLevel)
                ->pluck('id')->toArray();
        }
        ...
        $goods = rep()->good->getQuery()->select('product_id', 'type', 'related_type', 'ori_price', 'price',
            ...
            ->when($exceptCardIds, function ($query) use ($exceptCardIds) {
                $query->whereNotIn('related_id', $exceptCardIds);
