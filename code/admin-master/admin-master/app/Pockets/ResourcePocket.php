...
class ResourcePocket extends BasePocket
{
    ...
    public function appendPhotoToUsers(Collection $users)
    {
        ...
        foreach ($users as $user) {
            ...
            $status = false;
            $video  = $resources->where('type', Resource::TYPE_LIST[Resource::TYPE_VIDEO])->where('related_id',
                $user->id)->first();
            if ($video) {
    ...
    public function appendPhotoToUser(User $user, User $targetUser)
    {
        ...
        $user->setAttribute('photo',
            array_values($resources->where('type', Resource::TYPE_LIST[Resource::TYPE_IMAGE])->toArray()));
