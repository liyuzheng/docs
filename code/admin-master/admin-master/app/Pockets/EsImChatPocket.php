...
class EsImChatPocket extends EsPocket
{
    ...
    public function getImChatGroupBy(array $filters, array $ranges, string $groupByField)
    {
        if (!in_array($groupByField, ['send_id', 'receive_id'])) {
            return ResultReturn::failed("只能通过这些个字段分组");
        }
        $must = $query = $should = [];
        ...
        try {
            ...
            logger()->setLogType('es_group_by_error')->error($exception->getMessage());
        ...
        if (!isset($response['aggregations'][$groupByField]['buckets'])) {
            logger()->setLogType('es_group_by_no_result')->error('没有结果');

            return ResultReturn::failed('没有结果');
        }
        $buckets = $response['aggregations'][$groupByField]['buckets'];
        if (!count($buckets)) {
            logger()->setLogType('es_group_by_no_data')->error(json_encode($buckets));
