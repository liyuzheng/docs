...
class PaymentController extends BaseController
{
    ...
    public function goods(GoodsAndMemberRequest $request)
    {
        ...
        $goodsOs != Good::CLIENT_OS_IOS && $goods = pocket()->good
            ->judgeAndSetDiscount($user, $goods, $clientOs, $relatedType);
    ...
    public function discount(Request $request, int $uuid)
    {
        ...
        if (user_agent()->appName == 'com.xm.xiaoquan.app') {
            return api_rr()->getOK(
                ['discount' => 1, 'tips' => [['color' => '#FF9B06', 'txt' => '获取会员立享特权']]]);
        } elseif ($clientOs == Version::OS_STR_IOS) {
            $version = rep()->version->getRecordByOsAndVersionAndAppName(
                Version::OS_MAPPING[$clientOs], user_agent()->clientVersion, user_agent()->appName);
            if (!$version || !$version->audited_at) {
                return api_rr()->getOK(
                    ['discount' => 1, 'tips' => [['color' => '#FF9B06', 'txt' => '获取会员立享特权']]]);
            }
        }
