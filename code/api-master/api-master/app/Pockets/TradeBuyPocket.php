...
class TradeBuyPocket extends BasePocket
{
    ...
    public function sendUnlockMessage(User $consumer, User $beneficiary, array $relations, string $appName)
    {
        ...
        foreach ($relations as $relation => $price) {
            $relationsStr = ($relationsStr ? $relationsStr . trans('messages.and',
                        [], $beneficiary->language) : '') .
                trans(UserRelation::RELATIONS_MAPPING[$relation], [], $beneficiary->language);
        }

        $messageTemplate = trans('messages.beneficiary_unlock_notice_tmpl', [], $beneficiary->language);
        pocket()->user->appendToUsers(collect([$consumer, $beneficiary]), ['avatar']);

        $messages = [
            $consumer->uuid    => [
                'message' => sprintf(trans('messages.consumer_unlock_notice_tmpl', [],
                    $consumer->language), $beneficiary->nickname),
                'user'    => $beneficiary->get('uuid', 'nickname', 'avatar'),
            ],
            $beneficiary->uuid => [
                'message' => sprintf($messageTemplate, $consumer->nickname, $relationsStr),
    ...
    public function verifyUnlockVideoPermission(User $consumer, Resource $resource)
    {
        ...
        if (!$isReal || $isReal->status != UserPhoto::STATUS_OPEN) {
            return ResultReturn::failed(trans('messages.unlock_normal_video_error'),
        ...
        if ($isLooked && $isLooked->expired_at > time()) {
            return ResultReturn::failed(trans('messages.repeat_unlock_video_error'),
                ApiBusinessCode::FORBID_COMMON);
        }
     ...
     * @return ResultReturn
     */
    public function unlockUser(User $consumer, User $beneficiary, $relationType)
    {
        return DB::transaction(function () use ($consumer, $beneficiary, $relationType) {
            $consumerWallet     = rep()->wallet->getQuery()->where('user_id', $consumer->id)
                ->lockForUpdate()->first();
            $relationPricesResp = pocket()->userRelation->getUserSetUpRelationPrice($consumer,
                $beneficiary, $relationType);
            if (!$relationPricesResp->getStatus()) return  $relationPricesResp;
            $relationPrices = $relationPricesResp->getData();
