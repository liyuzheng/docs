...
class NotifyPocket extends BasePocket
{
     ...
     * @return array
     */
    public function getAliYunSmsConfig(string $appName, $area, int $mobile) : array
    {
        $allConfig = config('custom.aliyun');
        $config    = isset($allConfig[$appName]) ? $allConfig[$appName] : $allConfig['common'];
        $smsConfig = $config['sms'];
        $template  = $smsConfig['default_template'];
        $templates = $smsConfig['templates'];
    ...
    {
        [$template, $config] = pocket()->notify->getAliYunSmsConfig($appName, $area, $mobile);
