...
class BlacklistPocket extends BasePocket
{
    ...
    public function getBreakBlockUserId()
    {
        ...
        if (!Redis::exists($cacheKey)) {
            ...
            $lock->block(3, function () use ($cacheKey) {
                $values   = rep()->config->getQuery()->whereIn('key',
                    ['system_helper', 'netease_kf'])->pluck('value')->toArray();
                ...
                $excludeUserIds           = $users->pluck('id')->toArray();
                $unlimitedChatUsersConfig = rep()->config->getQuery()->where('key',
                    'unlimited_chat_users')->first();
                if ($unlimitedChatUsersConfig) {
                    $excludeUserIds = array_merge($excludeUserIds,
                        json_decode($unlimitedChatUsersConfig->value, true));
                }

                Redis::set($cacheKey, json_encode($excludeUserIds));
            });
        }
    ...
    {
        $black = rep()->blacklist->m()->where('related_type', Blacklist::RELATED_TYPE_OVERALL)->where('related_id',
            $userId)->first();
        if ($black) {
            $black->delete();
            $redisKey = config('redis_keys.blacklist.user.key');
        ...
        $black = rep()->blacklist->m()->where('related_type', Blacklist::RELATED_TYPE_CLIENT)->where('user_id',
            $userId)->first();
        if ($black) {
            $black->delete();
            $redisKey = config('redis_keys.blacklist.client.key');
