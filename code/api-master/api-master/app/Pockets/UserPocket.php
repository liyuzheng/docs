...
class UserPocket extends BasePocket
{
     ...
     * @return array
     */
    public function getDistanceToUsersByFixedLocationUUIDArr(Collection $users, float $selfLat, float $selfLng): array
    {
     ...
     * @return array
     */
    public function getDistanceToUsersByFixedLocationUserIdArr(Collection $users, float $selfLat, float $selfLng): array
    {
        $usersId     = $users->pluck('id')->toArray();
        $usersDetail = rep()->userDetail->m()
            ->select('user_id', 'lat', 'lng')
     ...
     * @return Collection
     */
    public function appendDistanceToUsersByFixedLocation(Collection $users, float $selfLat, float $selfLng)
    {
        $usersId     = $users->pluck('id')->toArray();
        $usersDetail = rep()->userDetail->m()
            ->select('user_id', 'lat', 'lng')
    ...
    {
        $users           = rep()->user->getByUUids(pocket()->util->getIosAuditUUIds(), 'id');
        $iosAuditUsersId = $users->pluck('id')->toArray();
        if (user_agent()->os == 'ios'
            &&
            !in_array(
                $user->id,
                array_merge(pocket()->blacklist->getBreakBlockUserId(), $iosAuditUsersId)
            )
        ) {
            $response = [
                [
                    'type'    => 100,
                    'value'   => 'https://web.wqdhz.com/banner?title=%E8%8B%B9%E6%9E%9C%E6%89%8B%E6%9C%BA%E7%94%A8%E6%88%B7%E5%BF%85%E7%9C%8B%21&url=https%3A%2F%2Fcdn1.wqdhz.com%2Fassets%2Fimages%2FzMMiwUhq2rjyQd.png',
                    'picture' => cdn_url('uploads/common/ios_counterfeit_notice_alert.png'),
                ]
            ];
...
//        }
        ...
        if ($unlimitedChatUsersConfig && in_array($user->id,
                json_decode($unlimitedChatUsersConfig->value, true))) {
            $response = [
                [
                    'type'    => 300,
                    'value'   => '',
                    'picture' => cdn_url('uploads/common/unlimited_chat_popup.png'),
                ]
            ];
    ...
    public function syncColdStartUser($userId)
    {
        ...
        $userPhotos    = DB::table('user_photo')->select('resource_id', 'related_type', 'status')
            ->where('user_id', $user->id)->where('deleted_at', 0)->get()->toArray();
        $userPhotos    = json_decode(json_encode($userPhotos), true);
        $newUserPhotos = [];
        ...
        $res      = ['resources' => $resources, 'photos' => $newUserPhotos];
