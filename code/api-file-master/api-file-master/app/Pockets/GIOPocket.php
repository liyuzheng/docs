...
class GIOPocket extends BasePocket
{
    ...
    public function __construct()
    {
        ...
        if (!isset($this->gioInstance)) {
            $this->gioInstance = GrowingIO::getInstance();
        }
    }
    ...
    public function reportUserPayMember($tradePay, $user)
    {
        ...
        $entryArr      = ['个人中心', '解锁私聊', '动态打招呼', '解锁微信', '发布动态', '查看谁看过我', '查看新入女生列表', '阅后即焚更长时间', '设置隐身', '回复私聊','查看资料','公众号','查看vip男生列表','私聊人数','阅后既焚更长时间'];
        $data          = [
            'VIPEntry_var'         => in_array($orderSceneArr->entry, $entryArr) ? $orderSceneArr->entry : '无',
            'VIPType_var'          => '无',
            'VIPPayType_var'       => '无',
        ...
        if ($orderSceneArr->reach_entry == '普通弹窗') {
