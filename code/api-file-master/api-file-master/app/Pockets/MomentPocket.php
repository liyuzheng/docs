...
class MomentPocket extends BasePocket
{
     ...
     */
    public function checkMoment($momentId)
    {
        ...
        if ($contentStatus && $resourceStatus) {
            rep()->moment->getById($momentId)->update(['check_status' => Moment::CHECK_STATUS_PASS]);
            pocket()->esMoment->updateMomentFieldToEs($momentId, ['check_status' => Moment::CHECK_STATUS_PASS]);
            pocket()->netease->msgSendMsg(config('custom.little_helper_uuid'), $user->uuid, '动态发布成功~');
        } else {
            rep()->moment->getById($momentId)->update(['check_status' => Moment::CHECK_STATUS_FAIL]);
            pocket()->esMoment->updateMomentFieldToEs($momentId, ['check_status' => Moment::CHECK_STATUS_FAIL]);
            $message = '';
    ...
    public function likeNetease(int $userId, int $likeUserId)
    {
        ...
        $message  = $msgCount > 1 ? sprintf(trans('messages.others_like_moment_notice_tmpl'), $likeUser->nickname, $msgCount)
