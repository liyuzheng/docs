...
class TradeBuyController extends BaseController
{
    ...
    public function unlockUser(Request $request, int $uuid)
    {
        ...
        $relationPricesResp = pocket()->userRelation->getUserSetUpRelationPrice($consumer, $beneficiary,
            $relationType);
        if (!$relationPricesResp->getStatus()) {
            return api_rr()->customFailed($relationPricesResp->getMessage(), $relationPricesResp->getData());
        }

        $relationPrices = $relationPricesResp->getData();
        $tranResp       = pocket()->tradeBuy->unlockUser($consumer, $beneficiary, $relationPrices);
