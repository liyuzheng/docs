    ...
    public function single(UploadsSingleRequest $request)
    {
        ...
        if ($watermark && in_array($type, ['user_avatar', 'user_photo'])) {
            ...
            try {
                ...
                logger()->setLogType('image_Invalid_dimensions')->error(json_encode($log_data));
        ...
        if ($type === 'user_photo' && $uuid) {
            ...
            try {
                ...
                logger()->setLogType('user_photo_check_url_' . $uuid)->error(json_encode($result));
                if ($result['code'] === ApiBusinessCode::SUCCESS) {
                    $checked = true;
                }
            } catch (\Exception $exception) {
                logger()->setLogType('user_photo_check_url_error_' . $uuid)->error($exception->getMessage());
                $checked = false;
            }
        }
